...
class ModalBorrarTurno extends Component {
  constructor(props){
    super(props);
    this.state={
      modal: false,            
      Id:this.props.turno.turnoId
    };      
  ...
  handleSubmit(event) {    
    ...
    const idTurno = this.state.Id;         
    let url = "http://localhost/gestiondeturnos/api/public/Turnos/borrarTurno/"+idTurno;
    let metodo = "DELETE";    
  fetch(url, {
          headers: {
  ...
  render() {  
    return (                                
        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
          <ModalHeader toggle={this.toggle}>Turno</ModalHeader>                                  
        <ModalBody>
                  <h3>¿Desea borrar el turno ?</h3>
        </ModalBody>
        <ModalFooter>              
...
export default ModalBorrarTurno;
